...
const columns = $computed(() => [
  ...
  { label: 'Level', name: 'level', filtered: persisted.filter.level != null },
  {
    label: 'Code',
    name: 'code',
    filtered: persisted.filter.code_prefix != null || persisted.filter.code_contains != null,
  },
  {
    label: 'Info',
    name: 'info',
  ...
  >
    ...
    <template #column-filter-code>
      <div style="min-width: 300px">
        <div class="q-mb-xs">
          <schema-form-base
            v-model="persisted.filter.code_prefix"
            :schema="{ title: 'Prefix', type: 'string', optional: true }"
          />
        </div>
        <schema-form-base
          v-model="persisted.filter.code_contains"
