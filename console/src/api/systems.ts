...
export type SystemRole = Zod.infer<typeof SystemRoleModel>
export const SystemRoleModel = Zod.enum(['connection', 'interface'])
...
export type SystemInfo = {
  ...
  subsystems: SystemInfo[]
}

export const SystemInfoModel: Zod.ZodType<SystemInfo> = Zod.object({
  name: Zod.string(),
  address: AddressModel,
  roles: Zod.array(SystemRoleModel),
  procedures: Zod.array(ProcedureInfoModel),
  subsystems: Zod.lazy(() => Zod.array(SystemInfoModel)),
}) as any
...
export const useSystems = defineStore('systems', () => {
  const client = useClient()
  const auth = useAuth()

  async function getSystem(address: Address) {
    try {
      return await client.get(`/api/systems/${address}`, {
        parse: SystemInfoModel,
      })
    } catch {
      return null
    }
  ...
  async function getProcedure(address: Address, procedure: string): Promise<ProcedureInfo | null> {
    try {
      return await client.get(`/api/systems/${address}/procedures/${procedure}`, {
  ...
  async function call(
    ...
    return await client.post(`/api/systems/${address}/procedures/${procedure}/call`, {
      data: unref(args),
  ...
  function useElementStream<TModel extends ZodTypeAny>(
    ...
    return client.useStream(
      computed(() => `/api/systems/${unref(address)}/procedures/${unref(query)}/subscribe`),
      // @ts-ignore
      ElementModel,
      onMessage,
  ...
  async function render(address: Address): Promise<RenderResult> {
    return await client.get(`/api/systems/${address}/procedures/render/call`, {
      parse: RenderResultModel,
    })
  }

  const query = useQuery({
    queryKey: computed(() => ['root-system', auth.user?.id ?? null]),
    queryFn: async () => {
      if (auth.user == null) {
        return null
      }

      return await getSystem(new Address('@'))
    },
  })

  const root = $computed<SystemInfo | null>(() => query.data.value ?? null)

  const mapping = $computed<Record<string, SystemInfo>>(() => {
    if (root == null) {
      return {}
    }

    const mapping: Record<string, SystemInfo> = {}

    function traverse(current: SystemInfo) {
      mapping[current.address.toString()] = current
      for (const child of current.subsystems) {
        traverse(child)
      }
    }
