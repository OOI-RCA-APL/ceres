import AJV from 'ajv'
import type { SchemaObject as BaseSchemaObject, ValidateFunction } from 'ajv'
...
import type { KeyInput } from '@/persistence'
import { useTime } from '@/time'
import type { MaybePromise, Plain } from '@/utilities'
...
export type SchemaFormOptions = {
  ...
  /** Draw the fields to sit inline, without the chrome a form lays around them.
  ...
  */
  ...
  onSubmit?: (value: any) => MaybePromise<SchemaFormState | undefined> | Promise<void>
...
export function useSchemaForm(options: SchemaFormOptions) {
  const hasModelValue = $computed(() => Object.hasOwn(options, 'value'))
  ...
  let state = $ref<SchemaFormState>(readonly ? 'viewing' : 'editing')

  const time = useTime()
  const persisted = usePersisted({
    schema: (zod) =>
      zod.object({
        value: zod.unknown().default(() => getInitialValue(rootSchema)),
      }),
  ...
  if (Object.hasOwn(options, 'initial')) {
    value.value = cloneDeep(options.initial)
  }
  ...
  const compilation = $computed<{
    ...
    try {
      return {
        validator: ajv.compile(rootSchema ?? true),
        error: null,
      }
    } catch (error) {
      return {
  ...
  const schemaErrorMessage = $computed(() => {
    ...
    return humanizeErrorMessage(schemaError.message)
  })
  ...
  function getInitialValue(pathOrSchema: SchemaPath | Schema | undefined = []): unknown {
    const schema: Schema | undefined =
      pathOrSchema == null
        ? undefined
        : Array.isArray(pathOrSchema)
          ? getSchema(pathOrSchema)
          : resolve(pathOrSchema)
    if (schema == null) {
      return undefined
    ...
    const type = (Array.isArray(schema.type) ? schema.type[0] : schema.type) ?? undefined
    if (type == null) {
      return null // JSON (Any)
    }
  ...
  const isEmpty = $computed(() => isEmptyObjectSchema(getSchema([])) && isEmptyObject(value.value))
  const isDefault = $computed(() => isEqual(value.value, getDefault()))
  const isInitialValue = $computed(() => isEqual(value.value, getInitialValue()))
  ...
  async function submit() {
    if (canSubmit && onSubmit) {
      state = 'submitting'
      try {
        state = ((await onSubmit(value.value)) as FormState | undefined) ?? 'editing'
      } catch {
        state = 'editing'
      }
    }
  ...
  function humanizeErrorMessage(message: string | undefined) {
