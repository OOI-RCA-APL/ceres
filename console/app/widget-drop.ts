...
function withoutHeld(bounds: RowBounds[], layout: WidgetRow[], held: Set<string>): RowBounds[] {
  ...
  for (const [index, measured] of bounds.entries()) {
    ...
    const left = (measured.widgets[0] as WidgetBounds).left
    const right = (measured.widgets[measured.widgets.length - 1] as WidgetBounds).right
    ...
    if (cells.length > 0) {
      ;(cells[cells.length - 1] as WidgetBounds).right = right
    }
  ...
  */
  ...
  useEventListener(window, 'pointerdown', (event: PointerEvent) => {
    ...
    if (
      ...
      !event.metaKey &&
      !event.ctrlKey &&
      target?.closest('[data-widget-header], [data-dismissable-layer], [role="dialog"]') == null
    ) {
