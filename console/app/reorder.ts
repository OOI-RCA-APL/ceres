...
export function usePointerReorder(options: {
  ...
  */
  ...
  function resolveTarget(delta: number): number {
    ...
    const held = placements[index] as Placement
    const center = held.start + held.size / 2 + delta

    let landing = index
    while (landing > 0) {
      const previous = placements[landing - 1] as Placement
      if (center >= previous.start + previous.size / 2) {
        break
      }

      landing--
    }
    while (landing < placements.length - 1) {
      const next = placements[landing + 1] as Placement
      if (center <= next.start + next.size / 2) {
        break
      }
  ...
  function shiftFor(index: number): number {
    ...
    const size = (drag.placements[drag.index] as Placement).size
  ...
  function settledOffset(): number {
    ...
    const held = placements[index] as Placement
    if (target > index) {
      const landing = placements[target] as Placement
      return landing.start + landing.size - (held.start + held.size)
    }
    if (target < index) {
      return (placements[target] as Placement).start - held.start
    }
