...
/// Return the `T` of a type spelled `Option<T>`, `None` for any other type.
...
pub fn expand(input: TokenStream) -> Result<TokenStream> {
    ...
    let classes = definitions.0.iter().map(ClassDefinition::expand);
    Ok(quote! { #(#classes)* })
}

impl ClassDefinition {
    /// The constructor parameters, one optional keyword argument per field.
    fn parameters(&self) -> Vec<TokenStream> {
        self.fields
            .iter()
            .map(|field| {
                let field_name = &field.name;
                let value = option_inner(&field.ty).unwrap_or(&field.ty);
                match &field.kind {
                    ...
                    FieldKind::Value => {
                        quote! {
                            #field_name: Option<<#value as crate::interop::PyFieldType>::Input>
                        }
                    }
                }
            })
    ...
    /// The raw-form initializer entries of the constructor, fields marked shared grouped
    /// into the raw form's flattened `shared` struct rather than assigned directly.
    fn assignments(&self) -> TokenStream {
        ...
        let direct_assignments = self
            ...
            .map(raw_assignment);
        let shared_assignments = self
            .fields
            .iter()
            .filter(|field| field.shared)
            .map(raw_assignment);
        let shared_entry = self.shared.as_ref().map(|shared_raw| {
            quote! { shared: #shared_raw { #(#shared_assignments,)* }, }
        });

        quote! { #(#direct_assignments,)* #shared_entry }
    }

    /// The property getters, one per field.
    ...
    /// Expand the class definition into its pyclass and implementations.
    fn expand(&self) -> TokenStream {
        ...
        let signature_entries = self.fields.iter().map(|field| {
            let field_name = &field.name;
